...
{
    interval_map<int, float> exon_interval_map;
    ...
    for(i=0; i<(int) start.size(); ++i){
        ...
        exon_interval_map += // element addition can also be done via operator +=
          make_pair( 
    ...
    {
        interval_map<int, float>::iterator it = exon_interval_map.begin();
        while(it != exon_interval_map.end())
        {
            interval<int>::type exon_interval = it->first;
            float psi_value = (*it++).second;
            output_file << first(exon_interval) << "\t" << upper(exon_interval) << "\t" << psi_value << endl;
        }
        output_file.close();
    }else
    {
